...
Скрипт по созданию пользователей AD и почтового ящика Exchange. Присутствует генератора пароля и транслит. Для заполнения списка серверов Exchange проверяется группа Exchange Servers, для получения списка Database и создания почтового ящика применяется icm (конфигурация Microsoft.Exchange). В начале скрипта указаны выпадающие списки Path OU и Company для их изменения и пополнения.
